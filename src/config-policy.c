...
static void append_rule(ParserState *state, PolicyRule *rule, gboolean connection_target, gboolean group_target, guint id) {
        GPtrArray *rules;

        if (connection_target) {
                rules = group_target ? rules_for_id(state->config->group_rules, id) : rules_for_id(state->config->user_rules, id);
        ...
        } else if (connection_target && group_target) {
                rules = rules_for_id(state->config->group_rules, id);
        } else {
        ...
        if (user || group) {
                ...
                if ((user && !g_str_equal(user, "*") && !lookup_uid(user, &id)) ||
                    (group && !g_str_equal(group, "*") && !lookup_gid(group, &id))) {
                        g_warning("Ignoring D-Bus policy rule for unknown %s '%s'", user ? "user" : "group", user ? user : group);
...
static void parser_start(void *data, const XML_Char *element, const XML_Char **attributes) {
        ...
        if (g_str_equal(element, "policy")) {
                ...
                name = "/usr/share/dbus-1/system-services";
                g_ptr_array_add(state->config->service_dirs, g_strdup(name));
                ...
                g_ptr_array_add(state->config->service_dirs, g_build_filename(g_get_user_data_dir(), "dbus-1", "services", NULL));
                for (const gchar * const *dirs = g_get_system_data_dirs(); *dirs; ++dirs)
                        g_ptr_array_add(state->config->service_dirs, g_build_filename(*dirs, "dbus-1", "services", NULL));
        }
}
...
static void load_directory(LauncherConfig *config, const gchar *path, GError **error) {
        GDir *directory;
        const gchar *name;
        GPtrArray *names;
        ...
        if (*value) {
                if (g_str_equal(element, "include")) {
                        ...
                        if (!load_file(state->config, path, state->include_ignore_missing, &error))
                                g_warning("Ignoring invalid D-Bus include %s: %s", path, error->message);
                        g_clear_error(&error);
                        g_free(path);
                } else if (g_str_equal(element, "includedir")) {
                        path = resolve_path(state->base_dir, value);
                        load_directory(state->config, path, &error);
                        if (error) {
                                g_warning("Ignoring invalid D-Bus include directory %s: %s", path, error->message);
                                g_clear_error(&error);
                        }
                        g_free(path);
                ...
                } else if (g_str_equal(element, "listen") && !state->config->address && g_str_has_prefix(value, "unix:path=")) {
...
static gboolean load_file(LauncherConfig *config, const gchar *path, gboolean ignore_missing, GError **error) {
        ...
        if (!XML_Parse(parser, contents, length, TRUE)) {
                g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE, "%s:%lu: %s", canonical,
                            (unsigned long)XML_GetCurrentLineNumber(parser), XML_ErrorString(XML_GetErrorCode(parser)));
        } else {
                success = TRUE;
        }
        ...
        for (guint array_index = 0; arrays[array_index]; ++array_index) {
                for (guint index = 0; index < arrays[array_index]->len; ++index) {
                        ...
                        if (rule->type == POLICY_RULE_CONNECT) {
                                ...
                                g_variant_builder_add(&own, "(btbs)", rule->allow, rule->priority, rule->own_prefix, rule->name ?: "");
                        } else if (rule->type == POLICY_RULE_SEND) {
                                g_variant_builder_add(&send, "(btssssuutt)", rule->allow, rule->priority, rule->name ?: "", rule->path ?: "", rule->interface ?: "", rule->member ?: "", rule->message_type, rule->broadcast, rule->min_fds, rule->max_fds);
                        } else if (rule->type == POLICY_RULE_RECV) {
                                g_variant_builder_add(&recv, "(btssssuutt)", rule->allow, rule->priority, rule->name ?: "", rule->path ?: "", rule->interface ?: "", rule->member ?: "", rule->message_type, 0, rule->min_fds, rule->max_fds);
                        }
                }
        }
...
GVariant *launcher_config_export_policy(LauncherConfig *config, gboolean user_scope, guint system_uid_max, const GArray *console_uids) {
        ...
        return g_variant_new("(@" UID_POLICY_TYPE "@" GID_POLICY_TYPE "@a(ss)bs)",
                             ...
                             g_variant_builder_end(&selinux), TRUE, user_scope ? "session" : "system");
}
